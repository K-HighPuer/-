...
function slideshow() {
  ...
  const imgs = focus.getElementsByTagName('img');
  const arrow_pre = document.getElementById('arrow-pre');
  const arrow_next = document.getElementById('arrow-next');
  const circle = document.getElementById('circle');
  let current = 0;  //当前所在图片的顺序，从0开始计
  let slideon = null; //自动播放中的定时器标识符
  let timer = null; //节流阀中的定时器标识符
  let flag = true; //节阀流
